...
>
> Historical plan: current transport authority is split across credential-free
> public registration and explicit custody, value, and quest keys. The canonical
> integration contract supersedes the single-key tasks below.
...
- [ ] **[P0] Add Azoa settings + typed HttpClients**
  - `ArdaNova.Infrastructure/Azoa/AzoaSettings.cs` — `BaseUrl`, scoped keys, trusted `TenantId`, `Mode`
  - Public registration sends no key; custody, value, and quest clients carry only their own scoped key
  - Register via `AddHttpClient` in `ArdaNova.Infrastructure/DependencyInjection.cs`
  - Bind config section `Azoa`; scoped keys come from the secret store and are never committed
