...
const DOMAIN_PATH = path.join(
  process.cwd(),
  "../ardanova-backend-api-mcp/api-server/src/ArdaNova.Domain/Models",
);
const DB_CONTEXT_PATH = path.join(
  process.cwd(),
  "../ardanova-backend-api-mcp/api-server/src/ArdaNova.Infrastructure/Data/ArdaNovaDbContext.cs",
);
const GENERATED_CONFIG_PATH = path.join(
  process.cwd(),
  "../ardanova-backend-api-mcp/api-server/src/ArdaNova.Infrastructure/Data/GeneratedModelConfigurations.cs",
);
...
const DELETE_BEHAVIOR_OVERRIDES: Record<
  string,
  "Cascade" | "SetNull" | "Restrict" | "NoAction"
> = {
...
const getNavPropertyName = (fkField: string, remoteModel: string): string => {
  let name = fkField.endsWith("Id") ? fkField.slice(0, -2) : remoteModel;
...
const buildMultiFKMap = (schema: Schema): Map<string, MultiFKRelation[]> => {
  ...
  const refsByPair: Map<string, { fkField: string; navName: string }[]> =
    new Map();
...
const buildBidirectionalFKs = (schema: Schema): BidirectionalFK[] => {
  ...
  schema.refs.forEach((ref) => {
    ...
    if (ep1.relation === "*" && ep2.relation === "1") {
      manyToOneRefs.push({
        fkHolder: ep1.tableName,
        target: ep2.tableName,
        fkField: ep1.fieldNames[0],
      });
    } else if (ep1.relation === "1" && ep2.relation === "*") {
      manyToOneRefs.push({
        fkHolder: ep2.tableName,
        target: ep1.tableName,
        fkField: ep2.fieldNames[0],
      });
    }
  });
  ...
  for (const refA of manyToOneRefs) {
    for (const refB of manyToOneRefs) {
      if (
        refA.fkHolder === refB.target &&
        refA.target === refB.fkHolder &&
        refA.fkHolder !== refA.target
      ) {
        ...
        if (!seen.has(pairKey)) {
          ...
          const collectionOnA =
            pluralize(refB.fkHolder) + (navB !== refA.fkHolder ? "" : "");
          const collectionOnB =
            pluralize(refA.fkHolder) + (navA !== refB.fkHolder ? "" : "");
...
const generateCSharp = async () => {
  ...
  if (bidirectionalFKs.length > 0) {
    console.log(
      `Detected ${bidirectionalFKs.length} bidirectional FK relationship(s):`,
    );
    bidirectionalFKs.forEach((bfk) =>
      console.log(
        `  ${bfk.tableA}.${bfk.fkFieldA} <-> ${bfk.tableB}.${bfk.fkFieldB}`,
      ),
    );
  }
...
`;
  ...
  if (fs.existsSync(enumDir)) {
    const existingEnumFiles = fs
      .readdirSync(enumDir)
      .filter((f) => f.endsWith(".cs"));
    const generatedEnumFileSet = new Set(
      generatedEnumNames.map((n) => `${n}.cs`),
    );
    const staleEnumFiles = existingEnumFiles.filter(
      (f) => !generatedEnumFileSet.has(f),
    );

    if (staleEnumFiles.length > 0) {
      if (noRemove) {
        console.log(
          `[--no-remove] Would remove stale enum files: ${staleEnumFiles.join(", ")}`,
        );
      } else if (dryRun) {
        console.log(
          `[DRY-RUN] Would remove stale enum files: ${staleEnumFiles.join(", ")}`,
        );
      } else {
...
`;
    ...
    table.fields.forEach((field) => {
      ...
      if (isNullable && csharpType !== "string" && csharpType !== "byte[]") {
        ...
        isNullable &&
        (csharpType === "string" || csharpType === "byte[]")
      ) {
        csharpType += "?";
      }
    ...
    tableRefs.forEach((ref) => {
      ...
      if (localEp.relation === "*" && remoteEp.relation === "1") {
        ...
        let relationName = fkField.endsWith("Id")
          ...
          : remoteModel;
        relationName =
          relationName.charAt(0).toUpperCase() + relationName.slice(1);
        ...
        const multiFKRelation = multiFKRelations?.find(
          (r) => r.fkField === fkField,
        );
        ...
        const multiFKForThisRef = multiFKRelationsToThis?.find(
          (r) =>
            r.sourceTable === remoteEp.tableName &&
  ...
  if (fs.existsSync(entityDir)) {
    const existingEntityFiles = fs
      .readdirSync(entityDir)
      .filter((f) => f.endsWith(".cs"));
    const generatedEntityFileSet = new Set(
      generatedModels.map((m) => `${m}.cs`),
    );
    const staleEntityFiles = existingEntityFiles.filter(
      (f) => !generatedEntityFileSet.has(f),
    );

    if (staleEntityFiles.length > 0) {
      if (noRemove) {
        console.log(
          `[--no-remove] Would remove stale entity files: ${staleEntityFiles.join(", ")}`,
        );
      } else if (dryRun) {
        console.log(
          `[DRY-RUN] Would remove stale entity files: ${staleEntityFiles.join(", ")}`,
        );
      } else {
        console.log(
          `Removing stale entity files: ${staleEntityFiles.join(", ")}`,
        );
...
`;
  ...
  if (bidirectionalFKs.length > 0) {
    ...
    bidirectionalFKs.forEach((bfk) => {
      const deleteBehaviorA =
        DELETE_BEHAVIOR_OVERRIDES[`${bfk.tableA}.${bfk.fkFieldA}`] ?? "SetNull";
      const deleteBehaviorB =
        DELETE_BEHAVIOR_OVERRIDES[`${bfk.tableB}.${bfk.fkFieldB}`] ?? "SetNull";
...
`;
  ...
  if (!dryRun) {
    ...
    console.log(
      `Generated ${compositeIndexes.length} composite index configurations`,
    );
  } else {
  ...
  if (fs.existsSync(DB_CONTEXT_PATH)) {
    ...
    if (modelsToRemove.length > 0 && !noRemove) {
      ...
      modelsToRemove.forEach((m) => {
        ...
        const removeRegex = new RegExp(
          `\\s*public\\s+DbSet<${m}>\\s+\\w+\\s*(?:=>\\s*Set<${m}>\\(\\)|{\\s*get;\\s*set;\\s*});?\\n?`,
          "g",
        );
    ...
    if (modelsToAdd.length > 0) {
      ...
      const dbSetInsertRegex =
        /public\s+DbSet<.*?>\s+.*?(?:=>\s*Set<.*?>\(\)|{\s*get;\s*set;\s*});?/g;
      ...
      if (matches.length > 0) {
        ...
        dbContext =
          dbContext.slice(0, insertionIndex) +
          newSets +
          dbContext.slice(insertionIndex);
...
generateCSharp().catch((error: unknown) => {
  console.error(error);
  process.exitCode = 1;
});
