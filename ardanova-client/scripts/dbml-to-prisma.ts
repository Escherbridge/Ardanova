...
// DO NOT EDIT DIRECTLY - modify the DBML file and run: npm run generate:dbml
  ...
  schema.tables.forEach((table) => {
    ...
    tableRefs.forEach((ref) => {
      ...
      if (localEp.relation === "*" && remoteEp.relation === "1") {
        ...
        relationFieldName = getUniqueRelationName(
          table.name,
          relationFieldName,
        );
        ...
        const pluralName = pluralize(
          remoteEp.tableName.charAt(0).toLowerCase() +
            remoteEp.tableName.slice(1),
        );
...
generatePrisma().catch((error: unknown) => {
  console.error(error);
  process.exitCode = 1;
});
