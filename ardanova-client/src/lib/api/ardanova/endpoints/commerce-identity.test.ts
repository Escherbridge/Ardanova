...
class CapturingClient extends BaseApiClient {
  readonly calls: Array<{ method: string; endpoint: string; body?: unknown; headers?: Record<string, string> }> = [];
  ...
  override post<T>(endpoint: string, body?: unknown, headers?: Record<string, string>): Promise<ApiResponse<T>> {
    this.calls.push({ method: "POST", endpoint, body, ...(headers ? { headers } : {}) });
    return Promise.resolve({ status: 200 });
  }
}
...
describe("commerce self-service API contracts", () => {
  it("uses actor-derived /me routes and identity-free request bodies", async () => {
    ...
    await new PayoutsEndpoint(client).requestPayout({ sourceTokenAmount: 10, holderClass: "CONTRIBUTOR" });
    await new TaskEscrowsEndpoint(client).getMine();
    await new TaskEscrowsEndpoint(client).create({ taskId: "task-1", shareId: "share-1", amount: 10 });
    ...
    await new OpportunityBidsEndpoint(client).getByOpportunityId("opportunity-1");
    await new OpportunityBidsEndpoint(client).accept("bid-1");
    await new TokenBalancesEndpoint(client).getBalance("config-1", "CONTRIBUTOR");
    await new TokenBalancesEndpoint(client).getArdaBalance();
    await new TokenBalancesEndpoint(client).getPortfolio();
    await new TokenBalancesEndpoint(client).checkLiquidity("config-1", "CONTRIBUTOR");
    await new FundingIntentsEndpoint(client).createCheckout(
      { projectTokenConfigId: "config-1", amount: "12.34", disclosureVersion: "funding-disclosure-v1" },
    ...
    expect(client.calls).toEqual([
      ...
      { method: "POST", endpoint: "/api/Payouts", body: { sourceTokenAmount: 10, holderClass: "CONTRIBUTOR" } },
      { method: "GET", endpoint: "/api/taskescrows/me" },
      { method: "POST", endpoint: "/api/taskescrows", body: { taskId: "task-1", shareId: "share-1", amount: 10 } },
      ...
      { method: "POST", endpoint: "/api/opportunity-bids/bid-1/accept", body: {} },
      { method: "GET", endpoint: "/api/TokenBalance/me/balance?projectTokenConfigId=config-1&holderClass=CONTRIBUTOR" },
      { method: "GET", endpoint: "/api/TokenBalance/me/arda" },
      { method: "GET", endpoint: "/api/TokenBalance/me/portfolio" },
      { method: "GET", endpoint: "/api/TokenBalance/me/liquidity?projectTokenConfigId=config-1&holderClass=CONTRIBUTOR" },
      {
        method: "POST",
        endpoint: "/api/funding-intents/checkout",
        body: { projectTokenConfigId: "config-1", amount: "12.34", disclosureVersion: "funding-disclosure-v1" },
        headers: { "X-Idempotency-Key": "9e68f25a-589c-472a-ab2d-0b4161c5ab89" },
      },
      { method: "GET", endpoint: "/api/funding-intents/funding-1" },
    ]);
    expect(client.calls.some(call => call.endpoint.includes("/tasks/user/"))).toBe(false);
  });
});
