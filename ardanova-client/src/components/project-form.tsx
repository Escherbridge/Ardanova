...
export function ProjectForm({ mode = "create", project }: ProjectFormProps) {
  const azoaStatus = api.azoaCustodialAccount.getStatus.useQuery(undefined, {
    enabled: mode === "create",
    retry: false,
  });
  ...
  useEffect(() => {
    if (mode === "edit" && project) {
      setFormData({
        ...
        resources: project.tags
          ? project.tags.split(",").filter((tag) => tag.trim())
          : [],
        ...
        attachments: {
          ...
          videos: project.videos
            ? project.videos
                .split(",")
                .filter((v) => v.trim())
                .map((v) => new File([], v.trim()))
            : [],
          images: project.images
            ? project.images
                .split(",")
                .filter((i) => i.trim())
                .map((i) => new File([], i.trim()))
            : [],
          documents: project.documents
            ? project.documents
                .split(",")
                .filter((d) => d.trim())
                .map((d) => new File([], d.trim()))
            : [],
        },
      });
    }
  ...
  const handleFileUpload = (
    type: keyof ProjectFormData["attachments"],
    files: FileList | null,
  ) => {
    if (files) {
      setFormData((prev) => ({
  ...
  if (mode === "create" && azoaStatus.isLoading) {
    return (
      <div className="flex min-h-72 items-center justify-center" role="status">
        <Loader2
          className="text-primary size-7 animate-spin"
          aria-label="Checking identity status"
        />
      </div>
    );
  }

  if (mode === "create" && azoaStatus.isError) {
    return (
      <div className="container mx-auto max-w-2xl px-4 py-16">
        <Card className="border-destructive border-2">
          <CardContent className="pt-6">
            <div className="flex flex-col items-center space-y-4 text-center">
              <div className="border-destructive bg-destructive/10 flex size-16 items-center justify-center border-2">
                <TriangleAlert
              ...
              <p className="text-muted-foreground max-w-md">
                ArdaNova could not confirm your current Azoa status, so project
                creation is paused rather than guessing. Your form has not been
                submitted.
              </p>
              <div className="flex flex-wrap justify-center gap-3 pt-2">
                <Button
                  type="button"
                  onClick={() => void azoaStatus.refetch()}
                  disabled={azoaStatus.isFetching}
                >
                  <RefreshCw
                    className={cn(
                      "mr-2 size-4",
                      azoaStatus.isFetching && "animate-spin",
                    )}
                    aria-hidden="true"
                  />
                  Retry status check
  ...
  // The API rechecks AZOA live; this client gate is guidance, not authority.
  if (mode === "create" && azoaStatus.data?.kycStatus !== "Approved") {
    return (
      <div className="container mx-auto max-w-2xl px-4 py-16">
        <Card className="border-system/30 border-2">
          <CardContent className="pt-6">
            <div className="flex flex-col items-center space-y-4 text-center">
              <div className="border-system/40 bg-system/10 flex h-16 w-16 items-center justify-center border-2">
                <Shield className="text-system h-8 w-8" />
              </div>
              <h2 className="text-foreground text-2xl font-black">
                Verification Required
              </h2>
              <p className="text-muted-foreground max-w-md">
                A current Azoa identity approval is required to create a
                project. ArdaNova checks the source directly instead of trusting
                a cached badge.
              </p>
      ...
      {successMessage && (
        <div className="rounded-none border border-green-200 bg-green-50 p-4">
          <p className="flex items-center gap-2 text-sm text-green-800">
            <span className="h-2 w-2 rounded-none bg-green-600"></span>
            {successMessage}
          </p>
        </div>
      )}
      ...
      {errors.submit && (
        <div className="rounded-none border border-red-200 bg-red-50 p-4">
          <p className="flex items-center gap-2 text-sm text-red-800">
            <span className="h-2 w-2 rounded-none bg-red-600"></span>
            {errors.submit}
          </p>
        </div>
      )}
              ...
              <div>
                ...
                {errors.title && (
                  <p className="mt-1 flex items-center gap-1 text-sm text-red-600">
                    <span className="h-1 w-1 rounded-none bg-red-600"></span>
                    {errors.title}
                  </p>
                )}
              </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <div className="flex h-8 w-8 items-center justify-center rounded-none bg-red-100">
          ...
          <CardContent className="space-y-4">
            <div>
              ...
              <Textarea
                ...
                onChange={(e) =>
                  handleInputChange("problem.description", e.target.value)
                }
            ...
            <div>
              ...
              <Textarea
                ...
                onChange={(e) =>
                  handleInputChange("problem.impact", e.target.value)
                }
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) =>
                  handleInputChange("problem.targetAudience", e.target.value)
                }
          ...
          <CardContent className="space-y-4">
            <div>
              ...
              <Textarea
                ...
                onChange={(e) =>
                  handleInputChange("solution.description", e.target.value)
                }
            ...
            <div>
              ...
              <Textarea
                ...
                onChange={(e) =>
                  handleInputChange("solution.approach", e.target.value)
                }
            ...
            <div>
              ...
              <Textarea
                ...
                onChange={(e) =>
                  handleInputChange("solution.expectedOutcome", e.target.value)
                }
        ...
        <Card>
          ...
          <CardContent className="space-y-4 px-4 sm:px-6">
            <div className="sm:col-span-2 lg:col-span-3">
              ...
              <div
                className={cn(
                  "mt-2 flex flex-wrap gap-2 rounded-md border p-3",
                    ...
                    <Input
                      ...
                      className={
                        errors.otherCategory
                          ? "border-red-500 pr-16 focus:border-red-500"
                          : "pr-16"
                      }
                    />
            ...
            <div>
              ...
              <div className="mt-2">
                <Input
                  ...
                  multiple
                  onChange={(e) =>
                    handleFileUpload("presentations", e.target.files)
                  }
